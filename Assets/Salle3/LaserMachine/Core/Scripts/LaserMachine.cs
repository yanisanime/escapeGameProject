        ...
        void OnEnable()
        {
            ...
            for (int i = 0; i < m_currentProperties.m_raysNumber; i++)
            {
                ...
                if (m_assignSparks)
                {
                    ...
                    sparks.transform.localScale *= 3; // Double the size of the sparks
                ...
                if (m_active)
                {
                    ...
                    if (m_currentProperties.m_physicsType == LaserProperties.PhysicsType.Physics3D)
                    {
                        ...
                        if (hitInfo3D.collider)
                        {
                            ...
                            if (m_assignSparks)
                            {
                                element.sparks.transform.position = hitInfo3D.point - hitInfo3D.normal * 0.5f;
                                element.sparks.transform.rotation = Quaternion.LookRotation(hitInfo3D.normal);
                            }


                            if (laserSoundObject == null)
                            {
                                Debug.LogWarning("laserSoundObject est NULL ! Réinitialisation...");

                                laserSoundObject = new GameObject("LaserImpactAudio");
                                laserAudioSource = laserSoundObject.AddComponent<AudioSource>();
                                laserAudioSource.clip = laserImpactSound;
                                laserAudioSource.spatialBlend = 1.0f;
                                laserAudioSource.loop = true;
                                laserAudioSource.playOnAwake = false;
                            }
                            // Suivre la position de l'impact
                            laserSoundObject.transform.position = hitInfo3D.point;
